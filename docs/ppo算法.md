PPO 算法训练流程详解
====================

本部分详细拆解 PPO 在大模型训练中的核心执行逻辑。

第一阶段：数据生成 (Rollout / Experience Collection)
--------------------------------------------------

此阶段为 Actor 模型 (Qwen2.5-3B) 与环境的交互过程：

1. **Prompt 输入**：从训练集中提取 Prompt。
2. **Actor 采样 (Sampling)**：Actor 根据当前权重 :math:`\theta_{old}`，对输入的 Prompt 进行自回归采样，生成对应的回复。
   
   * **注意**：除生成回复外，系统还会记录模型对每个 token 的对数概率 (Log Probabilities)，这是后续计算 ratio 的关键。
3. **Reward 打分 (Reward Modeling)**：
   
   * 将生成的回复送入 Reward 模型 (DeepSeek-v4-flash)。
   * Reward 模型输出标量分数 :math:`r_i`。
   * **最终奖励公式**：
     
     .. raw:: html
     
        $$R_i = r_i - \beta \cdot KL(Actor || Ref)$$
   
   * **作用**：防止 Actor 为骗取高分而产生乱码或偏离原始语言风格。

第二阶段：策略优化 (Policy Optimization / Gradient Update)
--------------------------------------------------------

本阶段利用样本的 :math:`R_i` 进行模型参数 :math:`\theta` 的更新：

1. 优势估计 (Advantage Estimation)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Critic 模型在此发挥作用：

* **价值评估**：Critic 对 80 个样本的当前状态评估一个价值 :math:`V_i`。
* **优势函数**：:math:`A_i = R_i - V_i`。
* **解释**：若 :math:`A_i > 0`，说明生成的回复优于 Critic 的预期，Actor 应增加生成该回复的概率。

2. 计算 PPO Loss (目标函数)
~~~~~~~~~~~~~~~~~~~~~~~~~~~

不再直接使用 :math:`A_i` 更新，而是通过对比新旧策略的 ratio：

.. raw:: html

   $$\text{ratio} = \frac{\pi_{\theta}(response|prompt)}{\pi_{\theta_{old}}(response|prompt)}$$

该 ratio 反映了当前模型对于“高分回复”的倾向程度。

**目标函数公式**：

.. raw:: html

   $$L(\theta) = \mathbb{E}[\min(\text{ratio} \cdot A_i, \text{clip}(\text{ratio}, 1 - \epsilon, 1 + \epsilon) \cdot A_i)]$$

* **Clip 的精髓**：强制 ratio 限制在 :math:`[1-\epsilon, 1+\epsilon]` 之间。
* **物理意义**：若新模型在某样本上改进过激，操作将进行截断，保证模型更新的平稳性。

3. 梯度反向传播 (Backward Pass)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

* 计算目标函数 :math:`L(\theta)` 对模型参数 :math:`\theta` 的梯度 :math:`\nabla_{\theta}L(\theta)`。
* 使用 Adam 等优化器执行梯度下降。
* **更新完成**：Actor 模型演进为更优版本 :math:`\theta_{new}`。
