...
2. **Actor 采样 (Sampling)**：Actor 根据当前权重 :math:`\theta_{old}`，对输入的 Prompt 进行 autoregressive (自回归) 采样，生成对应的回复。
...
3. **Reward 打分 (Reward Modeling)**：
   ...
   * Reward 模型输出标量分数 :math:`r_i`。
   * **关键处理**：不仅使用 :math:`r_i`，通常还会结合 Reference 模型 (冻结的初始模型) 计算 KL 散度惩罚。
   * **最终奖励公式**： :math:`R_i = r_i - \beta \cdot KL(Actor || Ref)`
   * **作用**：防止 Actor 为骗取高分而产生乱码或偏离原始语言风格。

第二阶段：策略优化 (Policy Optimization / Gradient Update)
--------------------------------------------------------

本阶段利用样本的 :math:`R_i` 进行模型参数 :math:`\theta` 的更新：
...
* **价值评估**：Critic 对 80 个样本的当前状态评估一个价值 :math:`V_i`。
* **优势函数**：:math:`A_i = R_i - V_i`。
* **解释**：若 :math:`A_i > 0`，说明生成的回复优于 Critic 的预期，Actor 应增加生成该回复的概率。

2. 计算 PPO Loss (目标函数)
~~~~~~~~~~~~~~~~~~~~~~~~~~~

不再直接使用 :math:`A_i` 更新，而是通过对比新旧策略的 ``ratio``：
...
* **Clip 的精髓**：强制 ``ratio`` 限制在 :math:`[1-\epsilon, 1+\epsilon]` 之间 (通常为 0.8 到 1.2)。
* **物理意义**：若新模型在某样本上改进过激，``ratio`` 超过范围，clip 操作将进行截断。这保证了模型更新的平稳性，避免因个别高分样本导致灾难性偏移。

3. 梯度反向传播 (Backward Pass)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

* 计算目标函数 :math:`L(\theta)` 对模型参数 :math:`\theta` 的梯度 :math:`\nabla_{\theta}L(\theta)`。
* 使用 Adam 等优化器执行梯度下降。
* **更新完成**：Actor 模型演进为更优版本 :math:`\theta_{new}`。
...
1. **Actor**：生成该回复的概率增大。
2. **Critic**：感知到该输入能带来高分，下次预测 :math:`V_i` 时会提高该状态的评分。
